...
const BrandHeader = () => {
  return (
    <div className="w-full pb-2 mb-2 flex flex-col items-center">
      {/* Zmiana: w-[98%] dla górnego paska ozdobnego nagłówka */}
      ...
      {/* Zmiana: w-[98%] i pogrubiony border-y-[2px] dla dolnego paska nagłówka */}
...
const CaseFile = ({ title, children, icon = '📄' }: { title: string, children: React.ReactNode, icon?: string }) => (
  <div className="my-8 border border-stone-400 bg-white/60 shadow-sm rounded-sm overflow-hidden break-inside-avoid text-left">
    ...
    {/* Zmiana: Usunięto teksturę subtle-paper.png z tła komponentu */}
...
export default function Page() {
  ...
  return (
    <>
      ...
      {/* Zmiana: Tło bg-[#e4dbbe] na całej głównej sekcji */}
      <main className="min-h-screen bg-[#e4dbbe] text-[#1c1917] selection:bg-yellow-900/20 font-serif flex flex-col items-center">
        {/* Zmiana: Usunięto zewnętrzne border-x z kontenera, usunięto warstwę "pixelozy" */}
        <div className="w-full max-w-5xl bg-[#e4dbbe] min-h-screen shadow-2xl flex flex-col relative overflow-hidden">

          <div className="w-full pt-0 px-6 flex flex-col items-center z-10 relative">
            <BrandHeader />
          </div>

          {/* NOWE PASKI PIONOWE: Specjalny kontener w-[96%] z border-x-[2px], który schodzi od razu po nagłówku do samego dołu */}
          <div className="w-[96%] mx-auto border-x-[2px] border-stone-900 flex-grow flex flex-col z-10 relative">
