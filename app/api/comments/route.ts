...
export async function GET(request: NextRequest) {
  ...
  const contentId = searchParams.get('contentId') || searchParams.get('slideId');
  ...
  try {
    const { comments, nextCursor } = await db.getComments(contentId, { limit, cursor, sortBy: sortBy || 'top', currentUserId });
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { text, parentId, imageUrl } = body;

    if (!contentId || (!text && !imageUrl)) {
      return NextResponse.json({ success: false, message: 'contentId and text or imageUrl are required' }, { status: 400 });
    }
    ...
    // Check if content exists (for now assuming it's a slide)
    ...
    const newComment = await db.addComment(contentId, currentUser.id!, sanitizedText, parentId || null, imageUrl || null);
