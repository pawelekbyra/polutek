...
import Image from 'next/image'; // Dodano dla optymalizacji LCP
...
const generateKordysPages = (count: number) => Array.from({ length: count }, (_, i) => `${KORDYS_IMAGES_URL}/30T 5 2021-1_page-${String(i + 1).padStart(4, '0')}.jpg`);
const generateBadiPages = (count: number) => Array.from({ length: count }, (_, i) => `${BADI_IMAGES_URL}/wyrok_page-${String(i + 1).padStart(4, '0')}.jpg`);

const GALLERY_NYDEK: GalleryData = { title: "Posiadłość w Nýdku (Archiwum)", images: [`${NYDEK_IMAGES_URL}/nydek01.jpg`, `${NYDEK_IMAGES_URL}/nydek02.jpg`, `${NYDEK_IMAGES_URL}/nydek03.jpg`, `${NYDEK_IMAGES_URL}/nydek04.jpg`, `${NYDEK_IMAGES_URL}/nydek05.jpg`, `${NYDEK_IMAGES_URL}/nydek06.jpeg`], signature: "LV 832" };
const GALLERY_WYROK_KORDYS: GalleryData = { title: "Pełne uzasadnienie wyroku: Jarosław K.", images: generateKordysPages(95), signature: "30 T 5/2021", pdfUrl: KORDYS_PDF_URL, type: 'verdict' };
const GALLERY_WYROK_BADI: GalleryData = { title: "Wyrok skazujący: Bartosz B.", images: generateBadiPages(3), signature: "66 T 146/2021", pdfUrl: BADI_PDF_URL, type: 'verdict' };
const GALLERY_WEZWANIE_KICINSKI: GalleryData = { title: "Wezwanie dla Michała Kicińskiego", images: [`${KORDYS_IMAGES_URL}/wezwanie/wezwanie_kicinski.png`], signature: "WD-I-3186/23" };
const GALLERY_JANOV: GalleryData = { title: "Dokumentacja Nieruchomości: Janov", images: Array.from({ length: 26 }, (_, i) => `${JANOV_IMAGES_URL}/janov${String(i + 1).padStart(2, '0') === '03' || String(i + 1).padStart(2, '0') === '05' ? '01' : String(i + 1).padStart(2, '0')}.jpg`), signature: "LV 127" };
...
const CaseFile = ({ title, children, type = 'evidence' }: { title: string, children: React.ReactNode, type?: 'evidence' | 'transcript' | 'email' }) => (
  <div className="my-10 group border-l-2 border-stone-800 bg-stone-50/40 p-6 transition-all hover:bg-white shadow-[4px_4px_0px_0px_rgba(28,25,23,0.05)]">
    ...
    <div className="font-mono text-sm leading-relaxed text-stone-700 italic border-l border-stone-200 pl-4">{children}</div>
  </div>
);
...
const PullQuote = ({ quote, author, source }: { quote: string, author: string, source: string }) => (
  <div className="my-16 text-center max-w-xl mx-auto">
    <p className="font-serif text-2xl md:text-3xl font-light italic text-stone-900 leading-tight mb-6">„{quote}”</p>
...
const LocationStamp = ({ name, code, plot, lv, onClick }: { name: string, code: string, plot: string, lv: string, onClick?: () => void }) => (
  <button onClick={onClick} className="my-8 w-full flex items-center gap-6 p-4 border border-stone-200 bg-white hover:border-stone-800 transition-all text-left group">
    <div className="bg-stone-100 p-4 group-hover:bg-stone-900 group-hover:text-white transition-colors"><HouseIcon className="w-6 h-6" /></div>
    <div>
      ...
      <main className="min-h-screen bg-[#F9F7F2] text-stone-900 selection:bg-stone-900 selection:text-white font-serif">
        <header className="pt-32 pb-16 px-6 border-b border-stone-200">
          <div className="max-w-4xl mx-auto">
            ...
            <h1 className="text-6xl md:text-8xl font-black tracking-tighter leading-[0.85] mb-8">ELIKSIR <br/> WIEDŹMINA</h1>
            <div className="max-w-2xl"><p className="text-2xl md:text-3xl text-stone-500 font-light leading-snug italic">Ayahuasca, miliardy z gier i śmierć, której nie było w scenariuszu.</p></div>
          </div>
        </header>
        ...
        <article className="max-w-2xl mx-auto px-6 py-20">
          <div className="prose prose-stone prose-lg max-w-none">
            ...
            <p>W obszernym i publicznie dostępnym uzasadnieniu <button onClick={() => openGallery('wyrok_kordys')} className="font-black border-b-2 border-stone-900 hover:bg-stone-900 hover:text-white px-1 transition-all">wyroku</button> Jarosława Kordysa pojawia się postać świadka Bartosza B.</p>
            ...
            <CaseFile title="Ustalenia Sądu ws. Własności">„...w odniesieniu do nieruchomości będących współwłasnością Bartosza B. i Michala D. K.”.</CaseFile>
            ...
            <p className="font-mono text-sm p-6 bg-stone-900 text-white leading-loose shadow-xl">
              Pobrany dokument pozostawia wątpliwości: w latach 2012–2023 współwłaścicielami nieruchomości byli:<br/>— Bartosz Badowski (10%)<br/>— <strong>Michał Dawid Kiciński (90%)</strong>
            </p>

            <h2 className="text-4xl font-black mt-24 mb-10 tracking-tighter uppercase">II. Na podsłuchu</h2>
            <p>Przełom w sprawie nastąpił dzięki policyjnej technice operacyjnej. Telefon Kordysa był na stałym podsłuchu. Sąd precyzyjnie rekonstruuje moment, w którym panika wdarła się do Janova:</p>
            <CaseFile title="Transkrypt: 24.08.2020" type="transcript">
              „oskarżony J. K. omawia z B., że dotarła do niego informacja, że w obiekcie w Janovie zmarła jakaś kobieta”.<br/><br/>Kordys: „W jakim zagrożeniu jest nasza praca?”
            </CaseFile>

            <div className="my-12 p-8 border-4 border-stone-900 bg-white">
              <p className="font-serif italic text-2xl leading-snug">„Z ich rozmowy wynika, że nie zajmowali się w zasadzie samym faktem śmierci, lecz raczej obawą, aby to nie przyciągnęło uwagi policji.”</p>
              <div className="mt-4 text-[10px] font-mono uppercase font-black">— Konkluzja Sądu Rejonowego</div>
            </div>

            <CaseFile title="Logistyka 'Herbaty'" type="transcript">„Następnie w rozmowie omawiają zamówienia «herbaty» z dżungli i to, czy im tego «nie zepsują», ekscytując się nagraniem od dostawcy, który «siedzi w dżungli i gotuje».”</CaseFile>

            <h2 className="text-4xl font-black mt-24 mb-10 tracking-tighter uppercase">III. Cena wolności</h2>
            <p>15 października 2020 roku sielankę przerwał huk granatów. Czeska specgrupa weszła do Kordysów.</p>
            <div className="my-12 group relative">
              <ArticleVideoPlayer src={`${PINATA_GATEWAY}/${ARREST_VIDEO_CID}/videoplayback.m3u8`} poster="" />
              <div className="mt-4 flex items-center gap-3 text-stone-500 font-mono text-[10px] uppercase"><Video className="w-4 h-4" /> Dowód Operacyjny: Nalot 15.10.2020</div>
            </div>

            <LegalNote term="Dohoda o vině a trestu">Ugoda o winie i karze. Pozwala na dobrowolne poddanie się karze w zamian za łagodniejszy wyrok, bez publicznego procesu. Bartosz Badowski skorzystał z tej drogi 2 listopada 2021 r.</LegalNote>
            <CaseFile title="Wyrok: Bartosz Badowski">„(...) co najmniej w 441 przypadkach zrealizował [...] pobyty [...] ukierunkowane na tzw. duchowe ćwiczenia spirytualne [...] udostępnił substancje zawierające DMT oraz marihuanę.”</CaseFile>
            ...
            <div className="my-12 flex flex-col items-center bg-stone-100 p-8 rounded-sm">
              <div className="relative w-64 h-80 shadow-2xl sepia hover:sepia-0 transition-all cursor-crosshair mb-4">
                <Image 
              ...
              <p className="text-[10px] font-mono font-black uppercase tracking-widest text-stone-400">Dowód #K-23: Wezwanie na przesłuchanie</p>
            </div>

            <CaseFile title="Korespondencja e-mail" type="email">„(...) Tak mogę zapłacić za swój błąd z Badim. Podaj mi Fundacje [...] i wpłacę tam dobrowolnie kwotę darowizny, w ramach Przeprosin wszechświatowi, za moją młodzieńczą naiwność i brak przenikliwości.”</CaseFile>
            ...
            <div className="my-12 p-8 border-l-8 border-stone-900 bg-stone-50">
               <ul className="list-none space-y-8 font-mono text-xs">
                <li className="flex gap-4"><span className="font-black text-stone-400 underline decoration-stone-200">21.09.2023</span><span>Odbiór wezwania przez Kicińskiego.</span></li>
                <li className="flex gap-4"><span className="font-black text-stone-400 underline decoration-stone-200">03.10.2023</span><span>Odkupienie 10% udziałów od Badowskiego. Scalenie własności.</span></li>
                <li className="flex gap-4"><span className="font-black text-stone-400 underline decoration-stone-200">11.10.2023</span><span>Przesłuchanie. Odmowa zeznań o przeszłości.</span></li>
                <li className="flex gap-4 p-4 bg-stone-900 text-white rounded-sm"><span className="font-black">23.10.2023</span><span>Zaledwie 12 dni po kłopotliwym przesłuchaniu, następuje telefon do Stefanka z propozycją oddania majątku.</span></li>
              </ul>
            </div>

            <TransactionStamp label="Katastr Nemovitostí: Darovizna" value="V-5821/2023-127" subDetails="Transfer majątku do stowarzyszenia Stefanka i Drzewińskiej." />

            <h2 className="text-4xl font-black mt-24 mb-10 tracking-tighter uppercase">VI. Nýdek</h2>
            <p>Janov nie był wyjątkiem. W Nýdku funkcjonował bliźniaczy ośrodek prowadzony przez Piotra Bonawenturę Tracza.</p>
            <LocationStamp name="NÝDEK" code="708186" plot="st. 506/1" lv="832" onClick={() => openGallery('nydek')} />
            
            {/* NAPRAWIONE ENCJE W LINII PONIŻEJ */}
            ...
            <p>Zbieżność tej sekwencji zdarzeń z &quot;darowizną&quot; Kicińskiego w Janovie pozwala dostrzec powtarzalny schemat wycofywania się właścicieli z infrastruktury powiązanej z nielegalnym procederem.</p>

            <TransactionStamp label="Sprzedaż Nieruchomości" value="V-2937/2021-832" subDetails="Marcin Iwiński sprzedaje posiadłość szamanowi Traczowi w czerwcu 2021 r." />
        ...
        <footer className="bg-stone-900 text-stone-400 py-24 px-6 font-mono text-xs uppercase tracking-widest">
          <div className="max-w-4xl mx-auto">
            <div className="grid md:grid-cols-2 gap-16">
              <div>
                <h3 className="text-white font-black mb-8 flex items-center gap-2"><ShieldCheck className="w-5 h-5" /> Weryfikacja Danych (Web3)</h3>
                <div className="space-y-4">
                  <a href={KORDYS_PDF_URL} target="_blank" rel="noreferrer" className="block hover:text-white transition-colors border-b border-stone-800 pb-2">30 T 5/2020: Jarosław K. (PDF)</a>
                  <a href={BADI_PDF_URL} target="_blank" rel="noreferrer" className="block hover:text-white transition-colors border-b border-stone-800 pb-2">66 T 146/2021: Bartosz B. (PDF)</a>
                  <a href="https://nahlizenidokn.cuzk.cz" target="_blank" rel="noreferrer" className="block hover:text-white transition-colors">Katastr Nemovitostí: LV 127 / LV 832</a>
                </div>
              </div>
