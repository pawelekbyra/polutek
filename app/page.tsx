...
const CaseFile = ({ title, children, type = 'evidence' }: { title: string, children: React.ReactNode, type?: 'evidence' | 'transcript' | 'email' }) => (
  <div className="my-10 border border-stone-400 bg-white shadow-sm rounded-sm overflow-hidden break-inside-avoid">
    <div className="bg-stone-200 border-b border-stone-300 px-4 py-2 flex items-center gap-2 text-xs font-mono text-stone-700 uppercase tracking-wider font-bold">
...
const LegalNote = ({ term, children }: { term: string, children: React.ReactNode }) => (
  <div className="my-10 flex gap-4 p-5 bg-blue-50 border-l-4 border-blue-900 rounded-r-sm shadow-sm">
    ...
    <div>
      ...
      <div className="text-stone-900 text-sm leading-relaxed font-medium">{children}</div>
    </div>
  </div>
);
...
const EvidenceVideo = ({ src, title, caption }: { src: string, title: string, caption: string }) => (
  <figure className="my-14">
    <div className="aspect-w-16 aspect-h-9 bg-black rounded-sm shadow-xl overflow-hidden relative group border border-stone-800">
      <iframe
        src={src}
        title={title}
