...
const CommentsModal: React.FC<CommentsModalProps> = ({ isOpen, onClose, contentId, initialCommentsCount }) => {
  ...
  const totalCommentCount = (contentId && commentCountChanges[contentId]) ?? initialCommentsCount;

  useEffect(() => {
    if (!isOpen || !contentId) return;
    const channel = ably.channels.get(`comments:${contentId}`);
    const onNewComment = () => queryClient.invalidateQueries({ queryKey: ['comments', contentId] });
  ...
  const likeMutation = useMutation({
    ...
    onMutate: async (commentId: string) => {
      await queryClient.cancelQueries({ queryKey: ['comments', contentId, sortBy] });
      const previousData = queryClient.getQueryData(['comments', contentId, sortBy]);

      queryClient.setQueryData(['comments', contentId, sortBy], (oldData: any) => {
    ...
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ['comments', contentId, sortBy] });
    },
  });
  ...
  const replyMutation = useMutation({
    ...
    onMutate: async ({ parentId, text, imageFile }) => {
      ...
      if (parentId) {
        ...
        await queryClient.cancelQueries({ queryKey: ['comments', contentId, 'replies', parentId] });
        const previousReplies = queryClient.getQueryData(['comments', contentId, 'replies', parentId]);
        queryClient.setQueryData(['comments', contentId, 'replies', parentId], (old: any) => {
        ...
        queryClient.setQueryData(['comments', contentId, sortBy], (old: any) => {
            if (!old) return old;
        ...
        await queryClient.cancelQueries({ queryKey: ['comments', contentId, sortBy] });
        const previousComments = queryClient.getQueryData(['comments', contentId, sortBy]);
        queryClient.setQueryData(['comments', contentId, sortBy], (old: any) => {
    ...
    onError: (err, { parentId }, context) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['comments', contentId], exact: false });
    },
    onSuccess: (data, variables) => {
      ...
      if (variables.parentId) {
          ...
          setTimeout(() => {
             queryClient.invalidateQueries({ queryKey: ['comments', contentId, 'replies', variables.parentId] });
          }, 1000);
      } else {
        setSortBy('newest');
        queryClient.invalidateQueries({ queryKey: ['comments', contentId, 'newest'] });
        if (sortBy !== 'newest') {
             queryClient.invalidateQueries({ queryKey: ['comments', contentId, sortBy] });
        }
        // Increment comment count in global state only for root comments
        useStore.getState().incrementCommentCount(contentId!, initialCommentsCount);
      }
      setNewComment('');
      setReplyingTo(null);
    },
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if ((!trimmedComment && !imageFile) || !user || !contentId) return;
