...
def verify_feed():
    with sync_playwright() as p:
        ...
        # Wait longer for the button because of 500ms timeout + potential render delays
        # Use get_by_text("Polski") which might be more robust than role if button structure is complex (though button is better)
        # In Preloader.tsx, it is a motion.button
        ...
        try:
            # Wait for ANY content in the virtual list
            # Virtuoso creates a scroller.
            # Let's wait for an image inside the slide
            page.wait_for_selector("div[data-test-id='virtuoso-item-list']", timeout=5000) # Virtuoso internal (maybe)

            # Or better, wait for our Slide component content.
            # Sidebar has icons.
